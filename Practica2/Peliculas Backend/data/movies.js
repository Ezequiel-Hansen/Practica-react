export const initialMovies = [
  {
    id: 1,
    title: "Argentina, 1985",
    director: "Santiago Mitre",
    duration: "140 min",
    rating: "Apta para mayores de 13 años",
    poster:
      "https://m.media-amazon.com/images/M/MV5BZDY2NWY3NDctYTViYi00YzAyLTgxYjEtNWFiYmFkMjk2NTRiXkEyXkFqcGc@._V1_FMjpg_UX1000_.jpg",
    cinemas: ["Cine “El Cairo”", "Showcase Rosario", "Cines del centro"],
    synopsis:
      "La historia real de los fiscales Julio Strassera y Luis Moreno Ocampo en el Juicio a las Juntas.",
    date: "2026-04-10",
    time: "20:00",
  },
  {
    id: 2,
    title: "Oppenheimer",
    director: "Christopher Nolan",
    duration: "180 min",
    rating: "Apta para mayores de 18 años",
    poster:
      "https://www.universalpictures.com.ar/tl_files/content/movies/oppenheimer/posters/01.jpg",
    cinemas: ["Cinemark Hoyts", "Showcase Rosario", "Las Tipas"],
    synopsis:
      "El físico J. Robert Oppenheimer trabaja con un equipo de científicos durante el Proyecto Manhattan.",
    date: "2026-04-11",
    time: "21:30",
  },
  {
    id: 3,
    title: "Spider-Man: Across the Spider-Verse",
    director: "Joaquim Dos Santos",
    duration: "140 min",
    rating: "Apta para todo público",
    poster:
      "https://m.media-amazon.com/images/M/MV5BMzI0NmVkMjEtYmY4MS00ZDMxLTlkZmEtMzU4MDQxYTMzMjU2XkEyXkFqcGdeQXVyMzQ0MzA0NTM@._V1_.jpg",
    cinemas: ["Nuevo Monumental", "Cinemark Hoyts", "Las Tipas"],
    synopsis:
      "Miles Morales se lanza a través del Multiverso para unir fuerzas con Gwen Stacy.",
    date: "2026-04-10",
    time: "17:00",
  },
  {
    id: 4,
    title: "The Dark Knight",
    director: "Christopher Nolan",
    duration: "152 min",
    rating: "Apta para mayores de 13 años",
    poster:
      "https://m.media-amazon.com/images/M/MV5BMTMxNTMwODM0NF5BMl5BanBnXkFtZTcwODAyMTk2Mw@@._V1_FMjpg_UX1000_.jpg",
    cinemas: ["Showcase Rosario", "Nuevo Monumental"],
    synopsis:
      "Batman tiene que mantener el equilibrio entre el heroísmo y el vigilantismo para detener al Joker.",
    date: "2026-04-12",
    time: "22:00",
  },
  {
    id: 5,
    title: "Toy Story 4",
    director: "Josh Cooley",
    duration: "100 min",
    rating: "Apta para todo público",
    poster:
      "https://labutacaweb.com/wp-content/uploads/2019/06/fb_img_15610376585733993096027120877199.jpg",
    cinemas: [
      "Cinemark Hoyts",
      "Showcase Rosario",
      "Las Tipas",
      "Nuevo Monumental",
    ],
    synopsis:
      "Woody, Buzz y el resto de la pandilla se embarcan en un viaje por carretera con un nuevo juguete llamado Forky.",
    date: "2026-04-10",
    time: "15:30",
  },
  {
    id: 6,
    title: "Inception",
    director: "Christopher Nolan",
    duration: "148 min",
    rating: "Apta para mayores de 13 años",
    poster:
      "https://m.media-amazon.com/images/M/MV5BMjAxMzY3NjcxNF5BMl5BanBnXkFtZTcwNTI5OTM0Mw@@._V1_.jpg",
    cinemas: ["Cines del centro", "Showcase Rosario"],
    synopsis:
      "Un ladrón que roba secretos corporativos a través del uso de la tecnología de intercambio de sueños.",
    date: "2026-04-11",
    time: "19:00",
  },
  {
    id: 7,
    title: "The Godfather",
    director: "Francis Ford Coppola",
    duration: "175 min",
    rating: "Apta para mayores de 18 años",
    poster:
      "https://m.media-amazon.com/images/M/MV5BNGEwYjgwOGQtYjg5ZS00Njc1LTk2ZGEtM2QwZWQ2NjdhZTE5XkEyXkFqcGc@._V1_.jpg",
    cinemas: ["Cine “El Cairo”", "Cines del centro"],
    synopsis:
      "El envejecido patriarca de una dinastía del crimen organizado transfiere el control de su imperio clandestino a su hijo reacio.",
    date: "2026-04-13",
    time: "20:30",
  },
  {
    id: 8,
    title: "Jurassic Park",
    director: "Steven Spielberg",
    duration: "127 min",
    rating: "Apta para mayores de 13 años",
    poster:
      "https://m.media-amazon.com/images/M/MV5BMjM2MDgxMDg0Nl5BMl5BanBnXkFtZTgwNTM2OTM5NDE@._V1_.jpg",
    cinemas: ["Showcase Rosario", "Cinemark Hoyts"],
    synopsis:
      "Un pragmático paleontólogo que visita un parque temático casi completo se ve encargado de proteger a un par de niños.",
    date: "2026-04-12",
    time: "16:00",
  },
  {
    id: 9,
    title: "Shrek",
    director: "Andrew Adamson",
    duration: "90 min",
    rating: "Apta para todo público",
    poster:
      "https://m.media-amazon.com/images/M/MV5BOGZhM2FhNTItODAzNi00YjA0LWEyN2UtNjJlYWQzYzU1MDg5L2ltYWdlL2ltYWdlXkEyXkFqcGdeQXVyMTQxNzMzNDI@._V1_.jpg",
    cinemas: ["Nuevo Monumental", "Las Tipas"],
    synopsis:
      "Un ogro malhumorado se embarca en una misión para rescatar a una princesa para un señor ambicioso.",
    date: "2026-04-10",
    time: "14:00",
  },
  {
    id: 10,
    title: "Pulp Fiction",
    director: "Quentin Tarantino",
    duration: "154 min",
    rating: "Apta para mayores de 18 años",
    poster:
      "https://upload.wikimedia.org/wikipedia/en/3/3b/Pulp_Fiction_%281994%29_poster.jpg",
    cinemas: ["Cine “El Cairo”", "Showcase Rosario"],
    synopsis:
      "Las vidas de dos sicarios de la mafia, un boxeador, la esposa de un gánster y dos bandidos se entrelazan.",
    date: "2026-04-14",
    time: "23:00",
  },
];